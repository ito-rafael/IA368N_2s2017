...
vu = 0;
omega = 0;
% vl = LeftWheelVelocity
% vr = RightWheelVelocity
[vl, vr ] = calculateWheelSpeeds(vu, omega, parameters)
Pioneer_p3dx_setWheelSpeeds(connection, vl, vr);

finish = 0;
while(!finish)
    switch (kbhit(1))
        %---------------------------------------------
        % print a linear forward speed
        case 'w'
            if (vu < 1.0)
                vu = vu + 0.1;
            else
                disp "linear speed already in maximum value!"
                fflush(stdout);
                pause(1);
            end
            % update robot's wheels' speed
            [vl, vr ] = calculateWheelSpeeds(vu, omega, parameters);
            Pioneer_p3dx_setWheelSpeeds(connection, vl, vr);
        %---------------------------------------------
        % print a rotational speed counter-clockwise
        case 'a'
            if (omega < 1.0)
                omega = omega + 0.1;
            else
                disp "rotational speed already in maximum value!"
                fflush(stdout);
                %pause(1);
            end
            % update robot's wheels' speed
        ...
        case 's'
            ...
            Pioneer_p3dx_setWheelSpeeds(connection, vl, vr);
        %---------------------------------------------
        % print a rotational speed clockwise
        case 'd'
            if (omega > -1.0)
                omega = omega - 0.1;
            else
                disp "rotational already in minimum value!"
                fflush(stdout);
                %pause(1);
            end
            % update robot's wheels' speed
        ...
        %---------------------------------------------
        % stop the robot
        case 32     % 32 is the ASCII code for "space"
            disp "\nrobot stopped!";
            fflush(stdout);
            vu = 0;
            omega = 0;
