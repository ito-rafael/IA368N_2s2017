% result = http_put(resource, payload)
%
% Input:
% - resource: the URL of the resource (protocol can be http or https)
% - payload: an object to put converted with savejson
%
% Possible outputs:
...
function result = http_put(resource, payload)
   ...
   % do http transaction
   content = handler.sendRequest('PUT', resource, 'application/json', pl);
   content = char(content);
   retcode = handler.getStatusCode;
   retcode = char(retcode);
   ctype = handler.getContentType;
   if strcmp(retcode, '200') == 1
      if strcmp(ctype, 'application/json') == 1 && length(char(content)) > 0
